...
static WORD r_offset[256], g_offset[256], b_offset[256];
static CLOSEST_INFO c_info;
static int tot_colors, pal_index;
static DWORD *squares;
...
BOOL dl1quant(BYTE *inbuf, BYTE *outbuf, int width, int height, int quant_to, int dither, BYTE userpal[3][256])
{
    ...
    if (build_table(inbuf, (DWORD)width * (DWORD)height) == 0)
    {
        dlq_finish();
        return 0;
    }
...
{
    DWORD i = 0, index = 0, cur_count = 0, head = 0, tail = 0;
    long j = 0;
...
static void fixheap(DWORD id)
{
    ...
    DWORD thres_index = heap[id].index, index = 0;
    DWORD half_totc = tot_colors >> 1;
    DWORD thres_val = rgb_table[thres_level][thres_index].pixels_in_cube;

    while (id <= half_totc)
    {
        index = id << 1;

        if (index < (DWORD)tot_colors)
    ...
    heap[id].index = (WORD)thres_index;
}
...
static void reduce_table(int num_colors)
{
    ...
    {
        BYTE tmp_level = heap[1].level, t_level = max(0,tmp_level - 1);
        DWORD tmp_index = heap[1].index, t_index = tmp_index >> 3;
...
static int quantize_image(BYTE *in, BYTE *out, int width, int height, int dither)
{
    if (!dither)
    {
        DWORD i = 0, pixels = width * height;
        WORD level = 0, index = 0;
        BYTE tmp_r = 0, tmp_g = 0, tmp_b = 0, cube = 0;
        BYTE *lookup=NULL;
    ...
    else // dither
    {
        ...
        BYTE *range_tbl = NULL, *range = NULL;
...
{
    DWORD i = 0, bestcolor = 0, curdist = 0, mindist = 0;
